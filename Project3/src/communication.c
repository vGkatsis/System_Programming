...
void sourceRead(int Fd, char **result){
...
void clientReadFromServer(int numOfWorkers, struct pollfd *serverFdArray, int *serverFd){
    ...
    while(readFromWorkers < numOfWorkers){                                      //While there are workers who havent responded to diseaseAggregator
        ...
        for(int i = 0; i < numOfWorkers; i++){                                  //Find the worker who has responded
            ...
            if(((serverFdArray[i].revents & POLLIN))){
                sourceRead(serverFd[i], &serverResponse);                    //Reade from worker
                commandResultsHandler(serverFd[i],serverResponse,&frequency,readFromWorkers,numOfWorkers - 1);
...
void readCommand(int clientFd, int serverFd,List **countriesList,hashTable *filePathsHashTable, hashTable *records, hashTable *countryHashTable, hashTable *diseaseHashTable, int bucketSize){

    char *command;

    //Read from a named pipe
    sourceRead(clientFd,&command);
    //Server gets command read from named pipe        
    serverReadCommand(command,clientFd,serverFd,countriesList,filePathsHashTable,records,countryHashTable,diseaseHashTable,bucketSize);
